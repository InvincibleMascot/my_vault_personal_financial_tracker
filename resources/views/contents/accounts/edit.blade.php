<x-app-layout>
        ...
        <div class="col-xl-12">
            <div class="card custom-card">
                ...
                <div class="card-body">
                    ...
                    <form id="accounts_edit_form">
                        @csrf

                        <input type="hidden" name="account_id" id="account_id" value="{{ base64_encode($record->id) }}">

                        <div class="row">
                            <div class="col-xl-3 mb-3">
                                <label class="form-label">Account Number <span class="text-danger">*</span></label>
                                <input type="text" name="account_number" id="account_number" class="form-control" maxlength="100" value="{{ $record->account_number ?? '' }}">
                            ...
                            <div class="col-xl-3 mb-3">
                                ...
                                <select name="account_type_id" id="account_type_id" class="form-control select2">
                                    ...
                                    @foreach($account_type as $type)
                                        <option value="{{ $type->id }}" {{ $record->account_type_id == $type->id ? 'selected' : '' }}>
                                            {{ $type->account_type }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-xl-3 mb-3">
                                ...
                                <input type="text" name="bank_name" id="bank_name" class="form-control" maxlength="100" value="{{ $record->bank_name ?? '' }}">
                                <span class="text-danger error-text bank_name_error"></span>
                            </div>

                            <div class="col-xl-3 mb-3">
                                <label class="form-label">Branch <span class="text-danger">*</span></label>
                                <input type="text" name="branch" id="branch" class="form-control" maxlength="100" value="{{ $record->branch ?? '' }}">
                                <span class="text-danger error-text branch_error"></span>
                            </div>

                            <div class="col-xl-3 mb-3">
                                <label class="form-label">IFSC Code <span class="text-danger">*</span></label>
                                <input type="text" name="ifsc_code" id="ifsc_code" class="form-control" maxlength="100" value="{{ $record->ifsc_code ?? '' }}">
                                <span class="text-danger error-text ifsc_code_error"></span>
                            </div>

                            <div class="col-xl-3 mb-3">
                                <label class="form-label">Balance <span class="text-danger">*</span></label>
                                <input type="number" name="balance" id="balance" class="form-control" min="0" step="0.01" value="{{ $record->balance ?? '' }}">
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#accounts_edit_form').on('submit', function (e) {
                e.preventDefault();

                clearValidationErrors();

                $('#accounts_submit_btn').prop('disabled', true).text('Updating...');
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        showAccountsAlert('success', res.message || 'Account updated successfully.');
                    ...
                    error: function (xhr) {
                        ...
                        if (xhr.status === 422 && xhr.responseJSON?.errors) {
                            ...
                            showAccountsAlert('error', xhr.responseJSON?.message || 'Unable to update account.');
                        }
                    },
                    complete: function () {
                        $('#accounts_submit_btn').prop('disabled', false).text('Update');
                    }
                });
            });
        });
